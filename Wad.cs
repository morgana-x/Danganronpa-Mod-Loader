...
namespace DanganronpaAnotherModLoader
{
    ...
                                 In this case all of them are folders, so there is no need to memorize it. */
                    ...
                    for (int i = 0; i < AmountOfFiles; i++)
                    {
                        Directory.CreateDirectory(System.IO.Path.Combine(DestinationDir, System.IO.Path.GetDirectoryName(FilesNames[i])));
                        ...
                        using (FileStream Extracted = new FileStream(System.IO.Path.Combine(DestinationDir, FilesNames[i]), FileMode.Create, FileAccess.Write))
        ...
        public static void RePackWAD(string FolderToRebuildToWAD, string DestinationDir)
        {
            ...
            using (FileStream FILEWAD = new FileStream(Path.Combine(DestinationDir, Path.GetFileName(FolderToRebuildToWAD) + ".wad"), FileMode.Create, FileAccess.Write))
            {
                using (BinaryWriter WADBinaryWriter = new BinaryWriter(FILEWAD))
                {
                    ...
                    Array.Sort(FullDirsAddress, new Common.AlphanumComparatorFast()); // Order alphanumerically the variable contents.
